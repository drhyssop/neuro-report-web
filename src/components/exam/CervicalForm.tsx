...
import type { CervicalExam, Bilateral, MotorGrade, DtrGrade, PathologicSigns } from '@/types/domain';
import type { DermatomePainEntry } from '@/types/domainV2';

interface Props {
  value: CervicalExam;
  yesterday?: CervicalExam;
  onChange: (next: CervicalExam) => void;
  /** 마운트 자동채움 전용 콜백 — 회진 확인(reviewed)으로 치지 않음 */
  onAutoInit?: (next: CervicalExam) => void;
...
export function CervicalForm({ value, yesterday, onChange, onAutoInit, dailyExtras, collapsibleNeuro, hideHistory }: Props) {
  // 디폴트 채움
  ...
  useEffect(() => {
    ...
    if (motorEmpty || dtrEmpty) {
      const next: CervicalExam = { ...value };
      if (motorEmpty) {
        const m: Record<string, Bilateral<MotorGrade>> = {};
        for (const g of MOTOR_GROUPS.cervical) m[g.key] = { lt: 5, rt: 5 };
        next.motor = m as CervicalExam['motor'];
      }
      if (dtrEmpty) {
        const d: Record<string, Bilateral<DtrGrade>> = {};
        for (const g of DTR_GROUPS.cervical) d[g.key] = { lt: 2, rt: 2 };
        next.dtr = d as CervicalExam['dtr'];
      }
  ...
  function patch(p: Partial<CervicalExam>) {
    onChange({ ...value, ...p });
  }
    ...
    <div className="space-y-4">
      ...
      <Section title="Pain">
        <div className="space-y-2">
          <PainBox
            title="Nuchal pain"
            active={!!value.nuchalPain}
            onActiveChange={(v) =>
              patch({
                nuchalPain: v || undefined,
                // 해제 시 main 자동 해제
                mainPain: !v && value.mainPain === 'nuchal' ? null : value.mainPain,
              })
            }
            vas={value.nuchalVas ?? null}
            onVasChange={(v) => patch({ nuchalVas: v })}
            isMain={value.mainPain === 'nuchal'}
            onMainChange={(next) => patch({ mainPain: next ? 'nuchal' : null })}
          />
          <PainBox
      ...
      <CollapsibleSection
        title="Motor power (Upper extremity, 기본 5/5)"
        collapsible={collapsibleNeuro}
        isNormal={motorStatus('cervical', value).normal}
        summary={motorStatus('cervical', value).summary}
      >
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {MOTOR_GROUPS.cervical.map((m) => (
            <BilateralRow
              key={m.key}
              label={m.label}
              myotome={m.myotome}
      ...
      <CollapsibleSection
        title="DTR (기본 2+)"
        collapsible={collapsibleNeuro}
        isNormal={dtrStatus('cervical', value).normal}
        summary={dtrStatus('cervical', value).summary}
      >
      ...
      <CollapsibleSection
        ...
        collapsible={collapsibleNeuro}
        isNormal={sensoryStatus('cervical', value).normal}
        summary={sensoryStatus('cervical', value).summary}
      >
        <SensorySection
          region="cervical"
          value={value.sensory}
